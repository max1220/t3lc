...
-- write REG_I to PERI0
-- generates 1(source is REG_I) or 2(other source) instructions
function PERI0_WRITE(source)
	source = source or "REG_I"
	assert_source(source)
	if source ~= "REG_I" then
		OP(source, "REG_I")
	end
	output(SPECIAL_OPS.PERI0_WRITE)
end

-- write REG_J to PERI1
-- generates 1(source is REG_J) or 2(other source) instructions
function PERI1_WRITE(source)
	assert_source(source)
	if source ~= "REG_J" then
		OP(source, "REG_J")
	end
	output(SPECIAL_OPS.PERI1_WRITE)
end
